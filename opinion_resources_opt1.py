...
def return_Nf(t, f, theta, data):
    ...
    if t == 0:
        ...
        return np.sum([f(t - s, theta) * data[:, s, :] for s in range(t)], axis=0).sum(
            axis=1
        )
...
def return_dNf(t, f, theta, alpha, data):
    ...
    if t == 0:
        ...
        return np.dot(
            ...
            np.sum(
                [
                    return_derivative_of_f_wrt_theta(t - s, theta) * data[:, s, :]
                    for s in range(t)
                ],
                axis=0,
            ).sum(axis=1),
...
def return_gradient_of_lambda_p(p, t, alpha, mu, nf, dnf, ext, data):
    ...
    P = data.shape[0]
    # parameter order is mu, alpha, theta
...
def return_likelihood_and_gradient(
    alpha, mu, theta, S, data, regularization, time_averaged=True
):

    factor = np.array([1, 1])  # scaling factor
    P = alpha.shape[0]
    ...
    for t in range(data.shape[1]):
        ...
        for p in range(P):
            ...
            log_likelihood += (n_pt * np.log(lambda_total[p]) - lambda_total[p]) * (
                1 / factor[p]
            )
    ...
    # regularize fit on mu and alpha
    # mu is on a different scale than alpha, rescale to dimensionless
    factor = np.sum(np.mean(data, axis=1), axis=1)
    reg = regularization * (
        np.sum((mu.reshape(-1) / factor) ** 2) + np.sum(np.abs(alpha.reshape(-1)) ** 2)
    )
...
def return_likelihood_and_gradient_from_flattened(
    plist, data, S, regularization, time_averaged
):
    P = data.shape[0]
    ...
    return return_likelihood_and_gradient(
        alpha, mu, theta, S, data, regularization, time_averaged
    )


def get_likelihood_and_gradient_of_set(
    samples, x0, S, regularization, time_averaged, how_long, how_many_samples
):
    ...
    for i in range(how_many_samples):
        nll, grad = return_likelihood_and_gradient_from_flattened(
            x0, samples[i, :, :how_long, :], S[:how_long], regularization, time_averaged
        )
        nlls.append(nll)
        grads.append(grad)
...
class MARKETSHAREMODELWITHSAMPLES(object):
    ...
    def __init__(
        ...
        obj, grad = get_likelihood_and_gradient_of_set(
            self.samples,
            p0,
            self.S,
            self.regularization,
        ...
        logging.basicConfig(
            filename=f"log/opt1:{logfit_label}.log",
            level=logging.INFO,
