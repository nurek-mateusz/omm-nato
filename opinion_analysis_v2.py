...
def generate_samples(
    plist, T_train, T_test, data, X, S, num_samples, P, M, K, base_X=None
):
    ...
    beta = plist[
        P * M + P * P + P * M * K + 1 : P * M + P * P + P * M * K + P * P * M * M + 1
    ].reshape(P, P, M, M)

    N_mean = np.mean(np.log(data[0, :, :T_train, :] + 1), axis=1)
    N_scale = np.std(np.log(data[0, :, :T_train, :] + 1), axis=1) + 1e-6

    if base_X is None:
        X_mean = np.mean(X[:, :T_train], axis=-1)
    ...
    for ns in range(num_samples):
        ...
        for t in range(1, T_train):
            ...
            T = return_T(
                t, gamma, beta, lambda_indpt, xf, N_mean, N_scale, X_mean, X_scale
            )
        ...
        for t in range(T_train, T_test):
            simulated_data = np.concatenate(
                [simulated_data, rs.reshape(P, 1, M)], axis=1
            )
...
def evaluate_fit(data, samples, T_train, T_test):
    ...
    return {
        "train_lvl1_tw_smape": train_level1_smape[0],
        "train_lvl2_tw_kl": train_tw_level2_entropy,
        "test_lvl1_tw_smape": test_level1_smape[0],
...
def get_average_shares_on_test(samples, T_train):
    tw_estimates = np.mean(samples[0, :, :, :], axis=-1)
    tw_estimates_df = pd.DataFrame(tw_estimates)
    twitteredf_normalized = tw_estimates_df.div(
        tw_estimates_df.sum(axis=1), axis=0
    ).values

    return (
        np.mean(twitteredf_normalized[T_train:, :], axis=0),
    )
